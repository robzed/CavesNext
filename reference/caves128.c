 ...
 *
 */
goroom(px,py,namepp,monhit,omonhit)
int px,py;
int *monhit,*omonhit;	/* pointer to monter hit points and pointer to
			   original monster hit pointer */
char **namepp;		/* this is the pointer to the name pointer */
{
 ...
 * - Player Death text -
 */
pdeath(hp,level,gold)
int hp,level,gold;	/* return type INT */
{
char c;
int go2;
...
mondeath(x,y,monname,oldhp,gold)
int x,y,oldhp,*gold;
char *monname;
{
...
/* - Cast option - */
castop(in,monhit,playerhp,pdamage)
int in,*monhit,*playerhp,*pdamage;
{
int i;
...
/* - Monster cast - */
moncast(x,y,namestr,manhp,mhit,level,mdamage)
int x,y,*manhp,*mhit,*level,*mdamage;
char *namestr;
{
int k;
...
/* - General fight - */
fighting(mdamage,pdamage,monhp,playerhp,name,moncount,nummon)
int mdamage,pdamage,*monhp,*playerhp,*moncount,*nummon;
char *name;
{
int k;
...
/* - Completed Mission - */
gend(gold,lev2,hp)
int gold,lev2,hp;	/* return type int */
{
int a;
...
x_rand(dice)
int dice;		/* return type this version int */
{
